...
namespace ArcGISRuntime.UWP.Samples.AccessLoadStatus
{
    public sealed partial class AccessLoadStatus
    {
        ...
        async private void OnMapsAndVisualizationsLoadStatusChanged(object sender, LoadStatusEventArgs e)
        {
            // Update the load status information
            await Dispatcher.RunAsync(CoreDispatcherPriority.Normal, () =>
                 {
                     loadStatusTextBlock.Text = string.Format("MapsAndVisualizations' load status : {0}", e.Status.ToString());
                 });
        }
    }
}
