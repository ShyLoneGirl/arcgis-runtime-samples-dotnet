...
namespace ArcGISRuntime.UWP.Samples.OpenMobileMapsAndVisualization
{
    public partial class OpenMobileMapsAndVisualization
    {
        ...
        private async Task<string> GetMmpkPath()
        {
            ...
            // Get the full path
            string filepath = Path.Combine(folder, "SampleData", "OpenMobileMapsAndVisualization", filename);

            // Check if the file exists
            if (!File.Exists(filepath))
            {
                // Download the map package file
                await DataManager.GetData("e1f3a7254cb845b09450f54937c16061", "OpenMobileMapsAndVisualization");
            }
            return filepath;
