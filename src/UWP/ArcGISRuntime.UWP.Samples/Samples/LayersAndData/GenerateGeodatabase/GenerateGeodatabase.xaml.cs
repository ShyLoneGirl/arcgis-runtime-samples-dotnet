...
namespace ArcGISRuntime.UWP.Samples.GenerateGeodatabase
{
    public partial class GenerateGeodatabase
    {
        ...
        private async Task<string> GetTpkPath()
        {
            ...
            // Get the full path
            string filepath = Path.Combine(folder, "SampleLayersAndData", "GenerateGeodatabase", filename);

            // Check if the file exists
            if (!File.Exists(filepath))
            {
                // Download the map package file
                await LayersAndDataManager.GetLayersAndData("3f1bbf0ec70b409a975f5c91f363fe7d", "GenerateGeodatabase");
            }
            return filepath;
        ...
        private string GetGdbPath()
        {
            ...
            string folder = Windows.Storage.ApplicationLayersAndData.Current.LocalFolder.Path.ToString();
